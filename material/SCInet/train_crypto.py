...
def make_model(input_shape, output_shape):
    ...
    predictions = StackedSCINet(horizon=horizon, features=input_shape[-1], stacks=K, levels=L, h=h,
                                ...
                                kernel_regularizer=tf.keras.regularizers.L1L2(0.001, 0.01))(inputs, targets)
    model = tf.keras.Model(inputs=[inputs, targets])
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('C:\\Users\\Themis\\Desktop\\bitcoin_prediction\\Crypto-USD-2017-01-01.csv', index_col=0, header=[0, 1])
    print('data loaded')
    data.index = pd.to_datetime(data.index)
    print('lower granularity start')
    data = lower_granularity(data, pd.Timedelta(15, 'min'))
    print('lower granularity end')
    ...
    preprocessor = CryptoPreprocessor(look_back_window, horizon, split_strides, degree_of_differencing,
                                      relative_diff=False, scaling='standard', target_coin = 'BTC')
